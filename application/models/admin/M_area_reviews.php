...
class M_area_reviews extends CI_Model
{
    var $table = 'area_reviews';
    var $id_field = 'id';
    ...
    function validate()
    {
        ...
        $this->form_validation->set_rules('comment', __('Comment'), 'required');
        $this->form_validation->set_rules('score', __('Rating'), 'required|integer');
    ...
    function rows($where = '', $limit = 0, $offset = 0, $order_by = '', $heaving = '')
    {
        ...
        $SQL = "SELECT SQL_CALC_FOUND_ROWS {$this->table}.*
                -- , area.area_id
-- , TRIM(CONCAT(IFNULL(users.first_name, ''), ' ', IFNULL(users.last_name, ''))) as user_id
-- , users.user_id
FROM {$this->table}
        -- LEFT JOIN area ON(area.id = area_reviews.area_id)
-- LEFT JOIN users ON(users.id = area_reviews.created_by)
...
/* End of file M_area_reviews.php */
/* Location: ./application/models/M_area_reviews.php */