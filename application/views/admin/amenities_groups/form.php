...
<form class="m-form m-form--fit m-form--label-align-right m-form--group-seperator-dashed" method="post" enctype="multipart/form-data"
      id="amenities_groups" action="<?php echo admin_url($this->_route . (!$row->id ? '/add' : '/update/' . $row->id)); ?>">
        <?php
        ...
        <div class="form-group m-form__group row">
                        <label class="col-lg-2 control-label"><?php echo __('Icon');?>:</label>             <div class="col-lg-6">
                                <div class="input-group">                <input type="text" name="icon" id="icon" class="form-control" placeholder="<?php echo __('Icon');?>" value="<?php echo htmlentities($row->icon);?>" />
