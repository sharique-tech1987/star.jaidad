...
class Blog_categories extends CI_Controller
{
    ...
    public function index()
    {
        ...
        /** -------- Query */
        $where = $this->where;
        $query = "SELECT blog_categories.`id`
  , _blog_categories.type AS `parent`
  , blog_categories.`type`
  , blog_categories.`ordering`
  , blog_categories.`status`
FROM {$this->table} AS blog_categories
LEFT JOIN blog_categories AS _blog_categories ON(_blog_categories.id = blog_categories.parent_id)
WHERE 1 {$where}";
