...
/**
 * Class booking * @property M_Modules $m_modules
 * @property M_cpanel $m_cpanel
 * @property M_booking $module
 * @property M_booking $m_booking
 * @property M_members $m_members
 * @property M_income $m_income
...
class Booking extends CI_Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method m_booking __construct
     * @model M_booking | m_booking
     * *****************************************************************************************************************
     */
    function __construct()
    {
        ...
        //TODO:: Check Login & Module -> booking
...
, TRIM(CONCAT(IFNULL(members.first_name, ''), ' ', IFNULL(members.last_name, ''))) as member
...
WHERE 1 {$where}";
    ...
    /**
     * *****************************************************************************************************************
     * @method booking ajax_grid | AJAX Grid | AJAX listing
     * *****************************************************************************************************************
     */
    ...
    public function view()
    {
        ...
        if ($id > 0) {
            $where = $this->where;
            $data['row'] = $this->module->row($id, $where);
            $data['booking'] = $this->m_members->bookings($data['row']->member_id, "AND booking.id='{$data['row']->id}'")[0];
    ...
    function AJAX($action, $id)
    {
        ...
        switch ($action) {
            case 'discount_coupon':
                $type = strtolower(getUri(6));

                if($type == 'apply') {
                    $coupon_code = getVar('coupon_code');
                    $coupon = $this->db->get_where('coupons', ['coupon_code' => $coupon_code], 1)->row();
                    if($coupon->id == 0){
                        $JSON['status'] = false;
                        $JSON['message'] = $coupon_code . ' is not valid!';
                    }
                    ...
                    save('booking', ['discount_coupon_id' => $coupon->id, 'discount_amount' => $discount_amount], "id='{$id}'");
                    activity_log('apply_coupon', 'booking', $id, 0, "Coupon ID: {$coupon->id} - {$discount_amount} - {$coupon_code}");
                } else if ($type == 'delete'){
                    $booking = $this->module->row($id);
                    if($booking->discount_coupon_id) {
                        $coupon = $this->db->get_where('coupons', ['id' => $booking->discount_coupon_id], 1)->row();

                        save('booking', ['discount_coupon_id' => 0, 'discount_amount' => 0], "id='{$id}'");
                        activity_log('delete_coupon', 'booking', $id, 0, "Coupon ID: {$booking->discount_coupon_id} - {$booking->discount_amount} - {$coupon->coupon_code}");
                    }
                }
                break;
            case 'delete_img':
    ...
    function get_payment($booking_id)
    {
        $this->load->model(ADMIN_DIR . "m_income");

        if ($this->m_income->validate()) {
            if ($id = $this->m_income->insert()) {
                set_notification(__('Record has been inserted'), 'success');

                $booking = $this->module->row($booking_id);

                $member_id = $booking->member_id;
                # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                # registration_email
                $member = $this->m_users->row($member_id);
                $msg = get_email_template(array_merge($member, $booking), 'Receive Payment');
            ...
            $__redirect = (!empty(getVar('__redirect')) ? getVar('__redirect') : admin_url($this->_route . "/view/{$booking_id}"));
            redirect($__redirect);
        } else {
            redirectBack();
        }
