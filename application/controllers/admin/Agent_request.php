 ...
 */
class Agent_request extends CI_Controller
{
    var $table;
    var $id_field;
    var $module;
    ...
    function __construct()
    {
        ...
        $this->where = " AND {$this->table}.become_agent = 1";
    ...
    function status()
    {
        ...
        $data = array('become_agent' => 0, 'user_type_id' => intval(get_option('agent_type_id')));

        $where = $this->id_field . " IN({$IDs}) " . $this->where;
        if (save($this->table, $data, $where)) {
            set_notification(__('Become an agent -> has been updated'), 'success');
